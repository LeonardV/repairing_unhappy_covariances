...
# =============================================================================
# FULL PIPELINE TEST (VCOVs)
# =============================================================================

cat("\n========== FULL PIPELINE TEST ==========\n\n")
t_total <- proc.time()["elapsed"]

# --- Step A: Load adversarial VCOVs ---
load("R_list.RData")
message(sprintf("Loaded %d VCOVs.", length(R_list)))

# --- Step B: Define SEM model ---
my_model <- '
  Y =~ V1 + V2 + V3
  X =~ V4 + V5 + V6
  Y ~ X
...
# --- Step C: Train detector (VCOVs) ---
message("\n--- STAGE 1: Training detector ---")
...
t1_done <- proc.time()["elapsed"] - t1
message(sprintf("Detector trained in %.1f sec\n", t1_done))

# --- Step D: non-converging matrix to repair ---
R_bad <- matrix(c(
...
# --- Step E: Run full repair ---
message("\n--- STAGE 3: Repairing non-converging matrix ---")
